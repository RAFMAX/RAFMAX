<div align="center">

```
 █████╗    ██████╗  █████╗  ██████╗ ██╗   ██╗███████╗
██╔══██╗   ██╔══██╗██╔══██╗██╔═══██╗██║   ██║██╔════╝
███████║   ██████╔╝███████║██║   ██║██║   ██║█████╗  
██╔══██║   ██╔══██╗██╔══██║██║   ██║██║   ██║██╔══╝  
██║  ██║██╗██║  ██║██║  ██║╚██████╔╝╚██████╔╝██║     
╚═╝  ╚═╝╚═╝╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝  ╚═════╝ ╚═╝     
```

### `> CS Student · Mobile Developer · Security Enthusiast`

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&size=14&pause=1000&color=00D4AA&center=true&vCenter=true&width=500&lines=Building+Flutter+apps;Web+Exploitation;Always+learning)](https://git.io/typing-svg)

</div>

---

## Tech Stack

<div align="center">

[![My Skills](https://go-skill-icons.vercel.app/api/icons?i=dart,flutter,java,python,c,html,css,js,threejs,bash,linux,blender,fastapi,mysql,firebase&theme=dark)](https://go-skill-icons.vercel.app)

</div>

---

## About Me
 
```bash
$ whoami
> A.Raouf — CS student from Algiers, Algeria
 
$ cat interests.txt
> Mobile development with Flutter & Dart
> Web security & CTF challenges
> Computer networking
> Backend architecture
 
$ cat mindset.txt
> I don't just write code — I study how systems are built,
  how they communicate, and how they break.
 
$ echo $CURRENT_GOAL
> Sharpen offensive security skills · Ship real Flutter apps · Keep learning
```

---

## Skills

<table>
<tr>
<td valign="top" width="25%">

**📱 Mobile**
- Flutter & Dart
- Firebase

</td>
<td valign="top" width="25%">

**🔐 Web Security**
- SQL Injection
- XSS · SSRF · SSTI
- JWT Attacks
- Burp Suite


</td>
<td valign="top" width="25%">

**🧊 3D Design**
- CAD
- Game Dev
- Archviz
- Product Design

</td>


</tr>
</table>

---


<div align="center">

`> Feel free to connect or collaborate`

[![GitHub](https://img.shields.io/badge/GitHub-rafmax-181717?style=flat-square&logo=github)](https://github.com/rafmax)
[![Email](https://img.shields.io/badge/Email-contact-00d4aa?style=flat-square&logo=gmail)](mailto:raouf.benoud@gmail.com)

</div>
